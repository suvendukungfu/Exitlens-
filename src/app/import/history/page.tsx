...
export default function ImportHistoryPage() {
  ...
  const getStatusBadge = (status: ImportBatch['status']) => {
    switch (status) {
      ...
      case 'failed':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
  ...
  return (
    <div className="pb-12">
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs rounded bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
                    ...
                    >
                      <td className="p-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded bg-blue-50 dark:bg-blue-950/60 flex items-center justify-center shrink-0">
                      ...
                      <td className="p-3.5 text-right">
                        {batch.errors.length > 0 ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-900/60 border border-rose-200 dark:border-rose-800 text-xs font-semibold transition-colors"
                          >
                          ...
                          <button
                            onClick={() => setSelectedBatch(batch)}
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-medium transition-colors"
                          >
                            <span>Details</span>
                          </button>
                        )}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/60">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  File: <code className="font-mono text-slate-700 dark:text-slate-300">{selectedBatch.fileName}</code>
                </p>
              </div>
              <button
            ...
            <div className="p-5 overflow-y-auto space-y-4 flex-1">
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="p-3 rounded-lg bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/60">
                  <span className="text-[10px] text-emerald-600 dark:text-emerald-400 uppercase font-semibold">Valid Rows</span>
                  <div className="text-xl font-bold text-emerald-700 dark:text-emerald-300 mt-0.5">
                    {selectedBatch.validRows}
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-rose-50/50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/60">
                  <span className="text-[10px] text-rose-600 dark:text-rose-400 uppercase font-semibold">Invalid Rows</span>
                ...
                {selectedBatch.errors.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                    {selectedBatch.errors.map((err, idx) => (
                      <div
                        key={idx}
                        className="p-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/50 text-xs flex items-start gap-2.5"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {err.employeeId && (
                              <span className="font-mono text-[11px] bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded">
                                {err.employeeId}
                              </span>
                            )}
                          </div>
            ...
            {/* Modal Footer */}
            <div className="p-3 border-t border-slate-200 dark:border-slate-800 flex justify-end bg-slate-50 dark:bg-slate-800/60">
              <button
                onClick={() => setSelectedBatch(null)}
                className="px-4 py-1.5 rounded-md bg-slate-900 text-white dark:bg-white dark:text-slate-900 text-xs font-semibold shadow-2xs hover:opacity-90 transition-opacity"
              >
                Close Retrospective
              </button>
            </div>
