  ...
  return (
    <div className="pb-16">
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        ...
        {/* STEP 1: Download Template */}
        {currentStep === 1 && (
          <div className="relative bg-white border border-slate-200/90 rounded-xl p-6.5 shadow-xs space-y-6 overflow-hidden">
            ...
            <div className="p-5 rounded-xl bg-blue-50 border border-blue-200 space-y-3">
              ...
              <ul className="text-xs text-blue-900 space-y-2 list-disc pl-5 leading-relaxed">
                <li>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
              ...
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-slate-100 hover:bg-slate-200/80 text-slate-700 text-xs font-semibold transition-all cursor-pointer active:scale-98"
              >
        ...
        {currentStep === 2 && (
          <div className="relative bg-white border border-slate-200/90 rounded-xl p-6.5 shadow-xs space-y-6 overflow-hidden">
            ...
            <div
              ...
              className="border-2 border-dashed border-slate-300 hover:border-blue-500 rounded-2xl p-12 flex flex-col items-center justify-center text-center cursor-pointer transition-all bg-slate-50 group"
            >
              <input
                ref={fileInputRef}
                type="file"
                ...
                className="hidden"
              />
              <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 mb-3.5 transition-transform duration-200 group-hover:scale-110">
        ...
        {currentStep === 3 && (
          <div className="bg-white border border-slate-200/90 rounded-xl p-14 text-center shadow-xs space-y-4">
          ...
          <div className="relative bg-white border border-slate-200/90 rounded-xl p-6.5 shadow-xs space-y-6 overflow-hidden">
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    File: <strong className="text-slate-700">{selectedFile?.name}</strong> • Scanned {validationResult.totalRows} data rows
                  </p>
                </div>
                ...
                {validationResult.errors.length > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold text-rose-700 bg-rose-50 border border-rose-200 hover:bg-rose-100 transition-all cursor-pointer"
                  >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3.5 text-xs">
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                ...
                <span className="text-2xl font-extrabold text-slate-900 mt-1 block font-mono tabular-nums">
                  {validationResult.totalRows}
                </span>
              </div>

              <div className="bg-emerald-50 p-3.5 rounded-xl border border-emerald-200">
                <span className="text-emerald-700 block font-bold text-[11px] uppercase tracking-wider">
                  Ready to Import
                </span>
                <span className="text-2xl font-extrabold text-emerald-700 mt-1 block font-mono tabular-nums">
                  {validationResult.validRows}
                </span>
              </div>

              <div className="bg-amber-50 p-3.5 rounded-xl border border-amber-200">
                <span className="text-amber-700 block font-bold text-[11px] uppercase tracking-wider">
                  Warnings
                </span>
                <span className="text-2xl font-extrabold text-amber-700 mt-1 block font-mono tabular-nums">
                  {validationResult.warningRows}
                </span>
              </div>

              <div className="bg-rose-50 p-3.5 rounded-xl border border-rose-200">
                <span className="text-rose-700 block font-bold text-[11px] uppercase tracking-wider">
                  Critical Errors
                </span>
                <span className="text-2xl font-extrabold text-rose-700 mt-1 block font-mono tabular-nums">
            ...
            {/* Preview Table */}
            <div className="overflow-x-auto border border-slate-200 rounded-xl max-h-72 shadow-2xs">
              {previewFilter === 'all' && (
                <table className="w-full text-xs text-left border-collapse">
                  <thead className="bg-slate-50 sticky top-0 text-slate-500 select-none">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {validationResult.parsedRecords.slice(0, 10).map((r) => (
                      <tr key={r.id} className="hover:bg-slate-50/60">
                        <td className="p-3 font-mono font-bold text-blue-600">{r.employeeId}</td>
                        <td className="p-3 text-slate-800 font-medium">{r.employeeName}</td>
                        <td className="p-3 text-slate-600">{r.department}</td>
                        <td className="p-3 text-slate-600">{r.plant.split(' ')[0]}</td>
                        ...
                        <td className="p-3 text-slate-800">{r.primaryReason}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="divide-y divide-slate-100">
                  {validationResult.errors.length === 0 ? (
                    ...
                    validationResult.errors.map((err, i) => (
                      <div key={i} className="p-3.5 text-xs flex items-start gap-2.5 bg-rose-50/60">
                        <AlertCircle className="w-4 h-4 text-rose-500 mt-0.5 shrink-0" />
                        <div>
                          <span className="font-bold text-rose-900">
                            Row {err.rowNumber} [{err.field}]:
                          </span>
                          <span className="ml-1.5 text-slate-700">{err.message}</span>
                        </div>
                      </div>
                    ))
                  )}
                ...
                <div className="divide-y divide-slate-100">
                  {validationResult.warnings.length === 0 ? (
                    ...
                    validationResult.warnings.map((w, i) => (
                      <div key={i} className="p-3.5 text-xs flex items-start gap-2.5 bg-amber-50/60">
                        <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                        <div>
                          <span className="font-bold text-amber-900">
                            Row {w.rowNumber} [{w.field}]:
                          </span>
                          <span className="ml-1.5 text-slate-700">{w.message}</span>
                        </div>
                      </div>
                    ))
                  )}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-200">
              <button
                onClick={resetWizard}
                className="text-xs text-slate-600 hover:text-slate-900 cursor-pointer font-medium"
              >
                Cancel & Re-upload
              </button>
          ...
          <div className="relative bg-white border border-slate-200/90 rounded-xl p-6.5 shadow-xs space-y-6 overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              >
                ...
                <div className="mt-3 text-[11px] font-semibold text-blue-700 space-y-0.5">
              ...
              <div
                ...
                className={`p-5 rounded-xl border cursor-pointer transition-all ${
                  importMode === 'replace'
                    ? 'border-rose-600 ring-2 ring-rose-500/20 bg-rose-50'
                    : 'border-slate-200 hover:border-slate-300 bg-white'
                }`}
              >
                <span className="font-bold text-slate-900 block mb-1 text-sm">
        ...
        {currentStep === 6 && (
          <div className="relative bg-white border border-slate-200/90 rounded-xl p-12 text-center shadow-xs space-y-6 overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-0.75 bg-linear-to-r from-emerald-500 to-teal-500" />
            <div className="w-16 h-16 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto shadow-sm shadow-emerald-500/20 ring-1 ring-emerald-500/30">
            ...
            <div className="flex flex-wrap justify-center gap-3 pt-2">
              ...
              <Link
                href="/import/history"
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 text-xs font-bold transition-all"
              >
              ...
              <Link
                href="/records"
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-all"
              >
      ...
      {showReplaceModal && validationResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-white backdrop-blur-xl border border-slate-200 rounded-2xl max-w-md w-full p-6.5 shadow-2xl space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center shrink-0 border border-rose-200">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-900 font-mono">{records.length} records</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">New records to commit:</span>
                <span className="font-bold text-emerald-600 font-mono">{validationResult.validRows} records</span>
              </div>
              <div className="flex justify-between pt-2 border-t border-slate-200">
                <span className="text-slate-500">Final dataset size:</span>
                <span className="font-bold text-blue-600 font-mono">{validationResult.validRows} records</span>
              </div>
            </div>
